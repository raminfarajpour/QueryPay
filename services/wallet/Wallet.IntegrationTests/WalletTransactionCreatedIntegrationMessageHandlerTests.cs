...
public class WalletTransactionCreatedIntegrationMessageHandlerTests(
    ReadModelFixture readModelFixture) : IClassFixture<ReadModelFixture>
{
    ...
    public async Task HandleAsync_ShouldConsumeMessagesAndSaveSuccessfully()
    {
        ...
        await PublishAsync(transactionEvent,
            ((EventBusSetting)eventBusSetting.Value).WalletIntegrationEventsExchange.WalletTransactionCreatedEventQueue.RoutingKey,
            CancellationToken.None);
    ...
    private async Task PublishAsync(IIntegrationEvent @event, string routingKey, CancellationToken cancellationToken)
    {
        ...
        await producerService.ProduceAsync(data,
            ((EventBusSetting)eventBusSetting.Value).WalletIntegrationEventsExchange.Name,
